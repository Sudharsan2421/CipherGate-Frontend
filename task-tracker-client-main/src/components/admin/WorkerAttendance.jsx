...
// FaSave icon import removed as it's no longer needed
...
// updateWorker import removed as salary update functionality is removed
...
const WorkerAttendance = () => {
    ...
    // isSaving state removed as salary update functionality is removed
    ...
    const downloadPDF = async (reportData) => {
        ...
        try {
            ...
            doc.rect(20, currentY - 5, 170, 10, 'F'); // Header background
            ...
            doc.text('Out Time', 75, currentY);
            doc.text('Worked', 100, currentY);
            doc.text('Permission (mins)', 125, currentY);
            doc.text('Deduction', 165, currentY);

            currentY += 5;
            doc.line(20, currentY, 190, currentY); // Header line
            currentY += 5;
            ...
            reportData.report.forEach((row, index) => {
                ...
                // Handle display for Sundays
                let inTimeText = row.inTime || '';
                let outTimeText = row.outTime || '';
                
                if (row.status === 'Sunday') {
                    inTimeText = 'Sunday';
                    outTimeText = 'Sunday';
                }

                doc.text(row.date || '', 22, currentY);
                doc.text(inTimeText, 50, currentY);
                doc.text(outTimeText, 75, currentY);
                doc.text(row.workedHours || '', 100, currentY);
                doc.text(row.permissionMins?.toString() || '0', 140, currentY, { align: 'center' });
                doc.text(cleanDeduction, 165, currentY);
    ...
    const fetchAllData = async () => {
        ...
        try {
            ...
            const filteredAttendance = Array.isArray(attendanceRes.attendance)
                ? attendanceRes.attendance.filter(item => item.worker?._id === id)
                : [];
    ...
    }, [fromDate, toDate, attendanceData, settingsData, fiteredBatch, leaves]);
    ...
    // Salary update functionality removed as requested
    ...
    return (
        <Fragment>
            ...
            <div className="flex justify-end space-x-4 items-center mb-6">
                ...
                {productivityData && (
                    <>
                {/* Update Salary button removed as requested */}
                        <Button
                            variant="success"
                            className="flex items-center"
                            onClick={() => downloadPDF({
