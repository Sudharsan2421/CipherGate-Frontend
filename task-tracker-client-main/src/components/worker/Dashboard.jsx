...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
        setIsLoading(true);
        try {
            if (user?._id && user.subdomain) {
                // Fetch profile, tasks, topics, columns, and notifications in parallel
                const [
                    profileData,
                    tasksData,
                    topicsData,
  ...
  // Prepare salary details only after data has been loaded
  const baseSalary = profile?.salary ?? 0;
  const finalSalary = profile?.finalSalary ?? 0;
